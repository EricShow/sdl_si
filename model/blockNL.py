...
class blockNL(torch.nn.Module):
    def __init__(self, channels, fs):    #(channels = 3 fs = 15)
    ...
    def forward(self, x):
        ...
        b, c, h, w = phi.size()
        phi_patches = F.unfold(phi, self.fs, padding=self.fs // 2)#[b, c*fs*fs, hw]  b,3*15*15 hw   将输入分块，hw是块的数量(因为有padding操作，所以输出是hw个，没有padding就是(h-ker0+1)*(w-ker1+1))，每个块的大小是c*kernal[0]*kernel[1]
        phi_patches = phi_patches.view(b, c, self.fs * self.fs, -1)#[b, c, fs*fs, hw],保持b,c,fs*fs维度，reshape
        ...
        att = torch.matmul(theta, phi_patches)# [b, h, w, 1, fs*fs]   猜测：张量的乘法，从后往前看，根据最少的计算量，进行乘法，前面维度一致那就表示个数，只在后面进行矩阵乘法
